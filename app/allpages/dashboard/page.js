...
const Dashboard = () => {
  ...
  const handleInvoice = () => {
    if (cartItems && cartItems.length > 0) {
      ...
      const sortedCart = [...cartItems].sort((a, b) => b.Date - a.Date);
      const latestCart = sortedCart[0]; // Most recent ticket

      console.log("Latest ticket:", latestCart);

      const prevInvoice = latestCart?.InvoiceNumber || "011039102000356913";

      // Remove the first leading zero explicitly
      const numericPart = prevInvoice.startsWith("0") ? prevInvoice.slice(1) : prevInvoice;

      // Increment using BigInt
      const incremented = (BigInt(numericPart) + 12n).toString();

      // Add back the leading zero manually
      const newInvoice = "0" + incremented;

      setInvoiceNumber(newInvoice);
    } else {
      // Default invoice number
      setInvoiceNumber("011039102000356913");
    }
  };

  const handleCashSaleNumber = () => {
    if (cartItems && cartItems.length > 0) {
      const sortedCart = [...cartItems].sort((a, b) => b.Date - a.Date);
      const latestCart = sortedCart[0];

      const prevCashSale = latestCart?.CashSale || "644162";

      // Random increment between 11 and 40
      const randomIncrement = Math.floor(Math.random() * (30 - 17 + 1)) + 11;

      // Increment and maintain length
      const incremented = (parseInt(prevCashSale) + randomIncrement)
        .toString()
        .padStart(prevCashSale.length, "0");

      setReceiptNumber(incremented);
    } else {
      // Default starting number
      setReceiptNumber("644162");
    }
  };
  ...
  const handleDateChange = (e) => {
    const isoDate = e.target.value; // "2025-05-30"
    const [year, month, day] = isoDate.split("-");
    const formatted = `${day}/${month}/${year}`; // "30/05/2025"
          ...
          >
            ...
            <div className="flex justify-around mt-4">
              <button
                ...
                onClick={() => { setreceiptDetailsModal(true), handleInvoice(), handleCashSaleNumber() }}
              >
                Receipt
              </button>
              <button
                ...
                onClick={() => { setSendModal(true), handleInvoice(), handleCashSaleNumber() }}
              >
                Send
              </button>
              <button
      ...
      `}
          >
            ...
            <div className="flex justify-between mb-2 font-bold">
              ...
              <div>
                <p className="text-sm">Cash Sale # :  MRCS{receiptNumber}</p>
              </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <div >
                        <div className="grid grid-cols-2 gap-4 w-40 text-right">
                          ...
                          <span>
                            {(Number(item.stock) * Number(item.Price)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </span>
            ...
            <div className="text-xs space-y-1">
              ...
              <p className="font-semibold">CU INV: {invoiceNumber}</p>
            </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-80">
          <div className={` p-6 rounded-xl shadow w-96 
            ...
            <div className="relative my-2">
              <input
                type="time"
                step="5" // 👈 This enables hours:minutes:seconds
                className="w-full p-3 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-[#303133] shadow-md"
                style={{ color: "#000000" }}
                value={selectedTime}
                onChange={(e) => setSelectedTime(e.target.value)}

...
const Ticket = () => {
    ...
    const handleEmployeeClick = (employeeName) => {
        ...
        if (ticket) {
            ...
            if (ticketsForEmployee.length === 0) {
                ...
                // Sort tickets by date in descending order (latest first)
                const sortedTickets = ticketsForEmployee.sort((a, b) => new Date(b.Date) - new Date(a.Date));

                console.log("employee before setting tickets", employeeName);
    ...
    const handleTicketClick = (cartItems, cashier, date, time, till, cashsale, invoice) => {
        setCart(cartItems);
                        ...
                        >
                            {filteredTickets && filteredTickets.map((ticket, index) => (
                                <button
                                    ...
                                    onClick={() => handleTicketClick(ticket.Cart, ticket.EmployeeID, ticket.ReceiptDate, ticket.ReceiptTime, ticket.ReceiptTill, ticket.CashSale, ticket.InvoiceNumber)}
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-80">
                    <div className="bg-white p-6 rounded shadow w-96" >
                        ...
                        <div className="mt-4 font-bold text-lg">
                            Total: Ksh {total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
